...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    num_classes = W2.shape[1] #输出层的类别
    one_hot_y = np.eye(num_classes)[y]
    for i in range(batch_cnt)
      start_idx = i * batch
      end_idx = min(start_idx+batch,X.shape[0])
      x = X[start_idx:end_idx,:]
      yy = one_hot_y[start_idx:end_idx,:]
      x_tensor = ndl.Tensor(x)
      y_tensor = ndl.Tensor(yy)
      first_logist = x_tensor @ W1
      first_output = ndl.relu(first_logist)
      second_logist = first_output @ W2
      loss_err = softmax_loss(second_logits, y_tensor)
      loss_err.backward()
